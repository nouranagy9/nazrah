...
3. **Dwell-time selection** — [`nazrah/dwell.py`](nazrah/dwell.py) selects
   ...
   eyelid control.
4. **Single-screen UI** — [`nazrah/ui.py`](nazrah/ui.py)'s `GridUI` is a
   Tkinter grid of gaze-selectable cells showing all 12 phrases at once
   (`main.py`'s `GRID_ITEMS`). An earlier version split this across a
   home screen (light toggle / go to needs) and a separate needs screen,
   dropped after live testing found the extra screen just got in the way
   of reaching the phrases that matter — `nazrah/light.py`'s relay control
   still exists (`GpioLightController`/`NoOpLightController`, same Pi/dev
   split as `camera.py`) but `main.py` doesn't call it anymore.
...
`main.py`'s `GRID_ITEMS` (what's actually shown) is built directly from
`phrases.PHRASES`, one cell per phrase — no extra cells to keep in sync.
